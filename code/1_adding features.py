...
merged_df = pd.merge(df1, df2[['Post code', 'Municipality code']], how='left', left_on='Zip Code', right_on='Post code')
...
merged_df = merged_df.merge(df3_grouped[['CD_REFNIS', 'MS_POPULATION']], how='left', left_on='nisCode', right_on='CD_REFNIS')
...
df4['Avg_Taxable_Income'] = round(df4['MS_TOT_NET_TAXABLE_INC'] / df4['MS_NBR_NON_ZERO_INC'], 2)
# Merging files by a common column with nis code
merged_df = merged_df.merge(df4[['CD_MUNTY_REFNIS', 'Avg_Taxable_Income']], how='left', left_on='nisCode', right_on='CD_MUNTY_REFNIS')
# Removing an unnecessary column
merged_df.drop(columns=['CD_MUNTY_REFNIS'], inplace=True)

# Adding Number_of_Buildings column
buildings_path = os.path.join(code_dir, '../input_data/building_stock_open_data_2024.csv')
...
# Merging files by a common column with nis code
merged_df = merged_df.merge(df5_grouped[['CD_REFNIS', 'MS_VALUE']], how='left', left_on='nisCode', right_on='CD_REFNIS')
# Removing an unnecessary column
merged_df.drop(columns=['CD_REFNIS'], inplace=True)
...
# Merging files by a common column with nis code
merged_df = merged_df.merge(df6_grouped[['CD_REFNIS', 'MS_TOTAL_TRANSACTIONS']], how='left', left_on='nisCode', right_on='CD_REFNIS')
# Removing an unnecessary column
merged_df.drop(columns=['CD_REFNIS'], inplace=True)
merged_df = merged_df.rename(columns={'MS_TOTAL_TRANSACTIONS': 'Number_of_Transactions'})
...
add_features_columns = ["Fully Equipped Kitchen", "Furnished", "Open Fire", "Terrace",
...
merged_df["Add_Fearures"] = merged_df["Add_Fearures"].apply(lambda x: "yes" if x else "no")
# Convert "Add_Fearures" into dummy variables
merged_df = pd.get_dummies(merged_df, columns=["Add_Fearures"], prefix="Add_Fearures", drop_first=True)
merged_df = merged_df.rename(columns={"Add_Fearures_yes": "Add_Fearures"})

# Drop unnecessary columns
columns_to_drop = ['Locality', 'Zip Code', 'nisCode', 'Fully Equipped Kitchen', 'Furnished',
                   'Open Fire', 'Terrace', 'Garden', 'Swimming Pool', 
                   'Terrace Area (m2)', 'Garden Area (m2)', 
