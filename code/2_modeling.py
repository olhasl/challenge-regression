...
class PricePredictor:
    def __init__(self, cat_features, output_dir, iterations=1000, learning_rate=0.05, depth=6):
        """
    ...
    def calculate_all_metrics(self, y_true, y_pred):
        ...
        smape = round(1/len(y_true) * np.sum(2 * np.abs(y_pred - y_true) / (np.abs(y_true) + np.abs(y_pred))), 4)
        return {"r2": r2, "mse": mse, "rmse": rmse, "mae": mae, "mape": mape, "smape": smape}
    ...
    def run_pipeline(self, df, target_column):
        ...
        """
        X_train, X_test, y_train, y_test = self.prepare_data(df, target_column, features_columns)
...
if __name__ == "__main__":
    ...
    target_column = 'Price'
    features_columns = ['Livable Space (m2)', 'Avg_Taxable_Income', 'Province', 'State of the Building', 'Subtype of Property_Grouped']
